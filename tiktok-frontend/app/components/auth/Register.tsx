...
export default function Register() {
  ...
  return (
    <>
      <div>
        <h1 className="mb-4 text-center text-[28px] font-bold">Register</h1>
        ...
        <div className="mt-6 px-6 pb-2">
          <button
            disabled={loading}
            onClick={() => register()}
            className={`flex w-full items-center justify-center rounded-sm py-3 text-[17px] font-semibold text-white ${!name || !email || !password || !confirmPassword ? 'bg-gray-200' : 'bg-[#F02C56]'} `}
          >
            {loading ? (
