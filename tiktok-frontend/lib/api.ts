...
export const videoAPI = {
  getFeed: (page = 1, limit = 10) => apiClient.get('/api/videos/feed', { params: { page, limit } }),
...
export const interactionAPI = {
  likeVideo: (videoId: string) => apiClient.post('/api/interactions/like', { videoId }),

  unlikeVideo: (videoId: string) => apiClient.post('/api/interactions/unlike', { videoId }),

  getLikeStatus: (videoId: string) => apiClient.get(`/api/interactions/like-status/${videoId}`),
  ...
  deleteComment: (commentId: string) => apiClient.delete(`/api/interactions/comment/${commentId}`),

  recordView: (videoId: string) => apiClient.post('/api/interactions/view', { videoId }),
};
